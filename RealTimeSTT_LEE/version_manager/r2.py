...
내부적으로 Queue는 잠금 및 세마포어를 사용하여 주어진 시간에 하나의 프로세스 또는 스레드만 put() 또는 get() 작업을 수행할 수 있도록 합니다.
    ...
    def _audio_data_worker(audio_queue,
        ...
        try:
            while not shutdown_event.is_set():
                try:
                    data = stream.read(buffer_size) # data type : byte, len : 1024 -> 512(samples) × 2(bytes) per sample=1024bytes. 16-bit integer (2 bytes per sample)
    ...
    def wait_audio(self):
        ...
        self.audio = audio_array.astype(np.float32) / INT16_MAX_ABS_VALUE # float32로 정규화 Whisper모델 입력 형식 매칭
