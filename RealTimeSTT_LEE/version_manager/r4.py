...
INIT_WAKE_WORD_BUFFER_DURATION = 3.0
            ...
            while True:
                try:
                    ...
                    stream = audio_interface.open(
                        rate=sample_rate,
                        format=pyaudio.paInt16, # bit depth 16bit -> 2byte
                        channels=1,
                        input=True,
                        frames_per_buffer=buffer_size, # 한 번에 처리할 오디오 프레임의 수를 결정
                        input_device_index=input_device_index
                    )
                    print("Microphone connection success\n")
    ...
    def text(self,
        ...
        if on_transcription_finished:
            ...
            threading.Thread(target=on_transcription_finished,
                             args=(self.transcribe().rstrip(".?!"),start_time,)).start()
        else:
            """
