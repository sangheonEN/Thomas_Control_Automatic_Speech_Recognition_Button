...
class AudioToTextRecorder:
    ...
    def __init__(self,
                 ...
                 device: str = "cuda",
                 on_recording_start=None, # 초기화 중에 on_recording_start 콜백이 제공된 경우 여기에서 호출됩니다. 이를 통해 녹음이 시작될 때 UI 업데이트나 로깅과 같은 외부 작업을 트리거할 수 있습니다.
    ...
    def _transcription_worker(conn,
        ...
        while not shutdown_event.is_set():
            try:
                if conn.poll(0.5):
                    # self.parent_transcription_pipe.send((self.audio, self.language)) 이 호출되면, 파라미터 값을 반환 받음.
            ...
            while not shutdown_event.is_set():
                try:
                    ...
                    if e.errno == pyaudio.paInputOverflowed:
                        ...
                        #         stream.stop_stream()
                        #         stream.close()
                        #         print("stream stop and close \n")
                        #     except OSError as e:
                        #         logging.warning(f"Failed to stop/close stream: {e}")
                        """
                        input_device_index = None
                        # print(f"Attempting to reconnect input_device_index {input_device_index}\n")
                        # restream start
    ...
    def wait_audio(self):
        ...
        # print(f"listen start : {self.listen_start}")
        ...
        print(f"self.reduce_noise_flag : {self.reduce_noise_flag}")
    ...
    #     audio_array = np.frombuffer(b''.join(self.frames), dtype=np.int16)
    #     self.audio = audio_array.astype(np.float32) / INT16_MAX_ABS_VALUE
    #     # print("wait_audio self.audio=audio_array.astype(np.float32) / INT16_MAX_ABS_VALUE\n")
    #     self.frames.clear()
        ...
        try:
            ...
            while self.is_running: # 오디오 입력을 지속적으로 확인
                ...
                if self.is_recording:
                    ...
                    # print("_recording_worker self.frames.append(data) \n")
